...
// function queryNilai(string $query)
// {
//   $connection = connect();
//   $datas = $connection->query($query);

//   return $datas->fetch_object();
// }
    ...
    <form action="mapel/store.php" method="post" class="relative w-full max-w-md max-h-full">
      ...
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <div class="px-6 py-6 lg:px-8 space-y-4">
          ...
          <div>
            <label for="sel" class="capitalize block mb-2 mt-3 text-sm font-medium text-gray-900 dark:text-white">Nama</label>
            <select id="sel" name="nama" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" fdprocessedid="7bq3m7">
              ...
              ?>
                <option value="<?= $data['nama_lengkap'] ?>"><?= ucfirst($data['nama_lengkap']) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div>
            <label for="kelas" class="capitalize block mb-2 text-sm font-medium text-gray-900 dark:text-white">kelas</label>
            <input type="text" name="kelas" id="kelas" placeholder="~" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required="" fdprocessedid="ju39cs">
          </div>
          <div>
            <label for="mapel" class="capitalize block mb-2 text-sm font-medium text-gray-900 dark:text-white">mapel</label>
            <input type="text" name="mapel" id="mapel" placeholder="~" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required="" fdprocessedid="ju39cs">
          </div>
